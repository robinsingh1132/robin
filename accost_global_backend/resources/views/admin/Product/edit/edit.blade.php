                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">
                                <i class="fas fa-plus-circle text-success"></i> Edit Product
...
@section('script')
    ...
    @include('admin.Product.edit.form-validate')
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            /*get categories form super categories*/
            $('#product_validate').on('change','#prod-sup-cat',function(){
                var prod_super_cat_id=$(this).val();
                getCategoriesFromSuperCategory(prod_super_cat_id);
            });
            $( document ).ready(function() {
                var prod_super_cat_id=$('#prod-sup-cat').val();
                getCategoriesFromSuperCategory(prod_super_cat_id);
            });
            function getCategoriesFromSuperCategory(prod_super_cat_id){
                var old_cat_id=$('#old_cat_id').val();
                var select_element = '';
               // console.log(old_cat_id);
                $.ajax({
                    type:'GET',
                    ...
                    success:function(response) {
                        ...
                        var html='';
                        html+=' <option>Please select categories</option>';
                        $.each(categories, function(index, value){
                            if(old_cat_id !=''){
                                if(old_cat_id==value.id){
                            ...
                            html+=' <option value="'+value.id+'" '+select_element+'>'+value.name+'</option>';
                        });
                        $('#prod-cat').empty();
                        $('#prod-cat').append(html);
                    }
                });
                var old_brand_id=$('#old_brand_id').val();
                $.ajax({
                    type:'GET',
                    url:"{{route('get-brand')}}",
                    data:{'prod_super_cat_id' : prod_super_cat_id ,'old_brand_id' : old_brand_id ,"_token": "{{ csrf_token() }}"},
                    success:function(response) {
                        var brands= response;
                        console.log(brands);
                        var html='';
                        ...
                        $.each(brands, function(index, value){
                            ...
                            html+=' <option value="'+value.id+'" '+select_element+'>'+value.brand_name+'</option>';
                        });
                        $('#prod-brand').empty();
            ...
            /* end get categories form super categories*/
            /*get subcategories form categories*/
            $('#product_validate').on('change','#prod-cat',function(){
                var prod_cat_id=$(this).val();
                getSubcategoriesFromCategory(prod_cat_id);
            });
            $( document ).ready(function() {
                var prod_cat_id=$('#old_cat_id').val();
                getSubcategoriesFromCategory(prod_cat_id);
            });
            function getSubcategoriesFromCategory(prod_cat_id){
                var old_subcat_id=$('#old_subcat_id').val();
                var select_element = '';
                $.ajax({
                    type:'GET',
                    ...
                    success:function(response) {
                        ...
                        var html='';
                        html+=' <option>Please select subcategories</option>';
                        $.each(subcategories, function(index, value){
                            if(old_subcat_id !=''){
                                if(old_subcat_id==value.id){
                                    select_element='selected';
                                }else{
                                    select_element='';
                                }
            ...
            $("#product_highlight").submit(function(e) { // for saving product highlight
                e.preventDefault();
                $('#ajax-loader').show();
                var form = $(this);
            ...
            $(document).on('click','.delete_size', function (e) {
                var current_parrent= $(this).closest('tr');
                let url = $(this).attr('data-link');
        ...
        function refreshTableSizes(arr_pro_size) {
            ...
            $.each(arr_unique_size, function(index, size) {
                ...
                let table_body = '<span class="required-label" style="margin:left;">*</span>(If you need to update sets please delete old set and create a new one.)<br>'+
                        '<div class="table-responsive mt-4"><h5 class="font-weight-bold"><span class="text-primary">Size:</span> ' + size + ', <span class="text-primary">Original Price:</span> ' + temp_price + '' +
